...
module.exports = {
  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    const guildPerms = message.client.commandPermissions.get(message.guild.id);
    const isDisabled = settings.commands?.get(cmd.name) === false || guildPerms?.get(cmd.name) === false;
    if (isDisabled) {
      return message.safeReply("This command is disabled on this server");
    }
    ...
    if (cmd.userPermissions && cmd.userPermissions?.length > 0) {
      ...
      if (missing.length > 0) {
        return message.safeReply(`You need ${parsePermissions(missing)} for this command`);
      }
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      ...
      if (missing.length > 0) {
        return message.safeReply(`I need ${parsePermissions(missing)} for this command`);
      }
    }

    message.client.logger.debug(`Permissions validated for command: ${cmd.name}`);
  ...
  handleSlashCommand: async function (interaction) {
    ...
    const guildPerms = interaction.client.commandPermissions.get(interaction.guild.id);
    const isDisabled = settings.commands?.get(cmd.name) === false || guildPerms?.get(cmd.name) === false;
    if (isDisabled) {
      return interaction.reply({
