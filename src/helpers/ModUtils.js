...
module.exports = class ModUtils {
  ...
  static async purgeMessages(issuer, channel, type, amount, argument) {
    ...
    const memberPerms = channel.permissionsFor(issuer);
    if (!memberPerms) {
      console.error("Failed to get member permissions");
      return "MEMBER_PERM";
    }
    
    const hasMemberPerms = memberPerms.has(["ManageMessages", "ReadMessageHistory"]);
    console.log(`Member permissions check: ${hasMemberPerms}`);
    if (!hasMemberPerms) return "MEMBER_PERM";

    // Check bot permissions
    const botPerms = channel.permissionsFor(channel.guild.members.me);
    if (!botPerms) {
      console.error("Failed to get bot permissions");
      return "BOT_PERM";
    }
    
    const hasBotPerms = botPerms.has(["ManageMessages", "ReadMessageHistory"]);
    console.log(`Bot permissions check: ${hasBotPerms}`);
    if (!hasBotPerms) return "BOT_PERM";
